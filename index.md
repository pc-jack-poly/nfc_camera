...
<p>Mobile payments today rely primarily on two technologies: NFC and optical barcodes. Although NFC provides better security, usability, and accessibility, nearly half of smartphones either lack NFC hardware or keep it disabled. As a result, many users are pushed toward barcode-based payments, despite their weaker security and less favorable user experience. In this work, we present MagCode+, a physical side-channel communication mechanism that allows an NFC reader to transmit data directly to a smartphone camera by producing standards-compliant barcodes through benign magnetic interference on the CMOS sensor. Unlike prior approaches that merely emulate barcode patterns, MagCode+ introduces a format-accurate modulation and decoding pipeline that outputs barcodes matching those expected by smartphone barcode decoders, while remaining fully consistent with established payment and retail protocols. We prototype both the NFC-side waveform engine and smartphone-side decoding pipeline and evaluate the system across 11 devices. MagCode+ natively supports six major 1D barcode standards, achieves up to 537bps throughput, and delivers widely used payment barcodes such as Code128 within 0.28s. These results demonstrate that MagCode+ forms an immediately deployable bridge between NFC infrastructure and non-NFC smartphones while requiring no modifications to existing barcode-based ecosystems.</p>
</div>
